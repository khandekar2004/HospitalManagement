...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        ...
        $qwr = mysqli_query($connect, "SELECT email FROM user where username ='$user'");
        $row=mysqli_fetch_array($qwr);
        $email=$row['email'];
    
                                    $sql="SELECT * FROM tblpatient WHERE PatientEmail ='$email'";
                    ...
                    if(mysqli_num_rows($result) < 1){
                        $output .= "<tr><td colspan=5 class='text-center'>No Data Found </tr></tr>";
                    }
                    while($ro=mysqli_fetch_array($result))
                    {   
                    $patName = $ro['PatientName'];
                       $patCont = $ro['PatientContno'];
                       $gender = $ro['PatientGender'];
                       $cdate = $ro['CreationDate'];
                       $udate =$ro['UpdationDate'];
                       $patId=$ro['ID'];
                    $output .="
                    ...
                    <td>
                     ...
                     <a href='viewPat.php?viewid=$patId'><i class='fa fa-eye '></i></a>
