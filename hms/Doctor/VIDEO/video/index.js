...
async function validateMeeting() {
  ...
  if (token != "") {
    ...
    if (result.meetingId === meetingId) {
      addMeetingIdToDatabase(meetingId);
...
function addMeetingIdToDatabase(meetingId) {
  // Create a new XMLHttpRequest object
  ...
  xhr.onload = function () {
    if (xhr.status === 200) {
      ...
      console.log("Meeting ID added to the database successfully.");
    } else {
      // Request failed
      console.log("Error adding Meeting ID to the database.");
    }
  };
